...
1. What’s the difference between an expression and a statement in C?
    Every statement does something, every expression evaluate to something.
3. Why can assignments like `x = y = z = 0;` be chained?
4. What is the return type of an expression like `x + 1.0` if `x` is `int`?
   Its depends in which type of variable u are using to save the result, if u using int result is 3 if x is 2, if u using float to save the result, the result gonna be 3.000000
6. Can a function return the result of another function? What is evaluated first?
   we can take add function use it in one other function and result of second function can be returned. Its gonna be evaluated in chronological order.
8. How does the compiler know the type of an expression?
   the compiler knows type of expression through analysis of deklarated variables, operators.
